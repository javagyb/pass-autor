...
public class HtmlDoclet extends AbstractDoclet {
	...
	protected DocData generateDocData(WRDoc wrDoc) {
		...
		for (String tag : tagList) {
			Map<String, List<OpenAPI>> taggedOpenAPIs = wrDoc.getTaggedOpenAPIs();
			result.getFacet_counts()
					.getFacet_fields()
					.addTagField(tag, taggedOpenAPIs.get(tag).size());
			Doc doc = new Doc();
			doc.setTag(tag);
			doc.setDescription(wrDoc.getDescription());
			if(CollectionUtils.isEmpty(taggedOpenAPIs.get(tag))){
				doc.setDetailApis(new ArrayList<>());
			}else {
			...
			for (OpenAPI openAPI : taggedOpenAPIs.get(tag)) {
				...
				api.setDescription(openAPI.getDescription());
				doc.getAPIs().add(api);
			}
			result.addDoc(doc);
		}
	...
	protected void generateWRNoFrameFile2(RootDoc root, WRDoc wrDoc)
			throws Exception {
		Map<String, Map<String, List<OpenAPI>>> tagMap = new HashMap<String, Map<String, List<OpenAPI>>>();
		System.out.println(wrDoc.getTaggedDescription()+"!!!!!!!!!!!!!!!!!");
